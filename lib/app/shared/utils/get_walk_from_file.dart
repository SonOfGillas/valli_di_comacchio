...
Future<Walk> getWalkFromGpxFile(String finalePathInAssets) async {
  final gpxFileContent = await rootBundle.loadString(finalePathInAssets);
  ...
  final profile = document.findAllElements('osmand:profile');
  ...
  if (profile.isEmpty) throw Exception('No profile found');
  ...
  for (var point in trackPoints) {
    ...
    if (currentElevation != null) {
      if (elevation > currentElevation) {
        upHill += elevation - currentElevation;
      } else if (elevation < currentElevation) {
        downHill += currentElevation - elevation;
      }
    } else {
  ...
  final type = profile.first.value?.toLowerCase() == 'pedestrian'
      ? WalkType.pedestrian
      : WalkType.bicycle;
